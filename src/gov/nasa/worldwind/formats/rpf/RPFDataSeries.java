...
//  DATA_SERIES_M2("M2", "MIM",          "Various",     "Military Installation Map (TBD #2)",          "CADRG",      -1),
    ...
    DATA_SERIES_MM("MM", "---", "Various", "(Miscellaneous Maps & Charts)", "CADRG", 12500),
...
//  DATA_SERIES_IV("IV", "---",          "Various>10m", "Imagery, greater than 10 meter resolution",   "CIB",        -1),
    ...
    {
        if (rpfDataType == null || !isRPFDataType(rpfDataType))
        {
        ...
        if (scaleOrGSD <= 0)
        {
            String message = Logging.getMessage("RPFDataSeries.InvalidScaleOrGSD", scaleOrGSD);
            Logging.logger().fine(message);
            throw new IllegalArgumentException(message);
        }
    ...
    private static String[] RPF_DATA_TYPES;
    ...
    public static boolean isRPFDataType(String rpfDataType)
    {
        ...
        if (RPF_DATA_TYPES == null)
            RPF_DATA_TYPES = new String[] {CADRG_DATA_TYPE, CIB_DATA_TYPE};
