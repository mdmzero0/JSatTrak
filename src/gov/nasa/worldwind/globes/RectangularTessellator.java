...
/**
 * @author tag
 * @version $Id: RectangularTessellator.java 3683 2007-12-03 19:49:04Z tgaskins $
 */
abstract public class RectangularTessellator extends WWObjectImpl implements Tessellator
{
    protected static class RenderInfo
    {
        ...
        protected RenderInfo(int density, DoubleBuffer vertices, DoubleBuffer texCoords, Vec4 refCenter, int resolution)
        {
    ...
    protected static class CacheKey
    {
        ...
        protected CacheKey(Sector sector, int resolution, int density)
        {
            this.sector = sector;
            this.resolution = resolution;
            this.density = density;
    ...
    abstract protected Iterable<RectTile> getTopLevelTiles(DrawContext dc);

    abstract protected RectTile createTile(DrawContext dc, Sector tileSector, int level);

    abstract protected RenderInfo buildVerts(DrawContext dc, RectTile tile, int resolution, boolean makeSkirts);

    abstract protected int getTargetResolution(DrawContext dc, RectTile tile);

    abstract protected boolean acceptResolution(DrawContext dc, RectTile tile, int resolution);
    ...
    private int maxLevel = DEFAULT_MAX_LEVEL; // TODO: Make configurable

    protected RectangularTessellator.CacheKey createCacheKey(DrawContext dc, RectTile tile, int resolution)
    {
        return new CacheKey(tile.sector, resolution, tile.density);
    }

    public Sector getCurrentCoverage()
    {
    ...
    public SectorGeometryList tessellate(DrawContext dc)
    {
        ...
        String cacheId = this.getCacheId();//SectorGeometry.class.getName();
        if (cacheId != null && !WorldWind.getMemoryCacheSet().containsCache(cacheId))
        {
            long size = Configuration.getLongValue(AVKey.SECTOR_GEOMETRY_CACHE_SIZE, 20000000L);
            MemoryCache cache = new BasicMemoryCache((long) (0.85 * size), size);
            cache.setName(this.getCacheName());
            WorldWind.getMemoryCacheSet().addCache(cacheId, cache);
        }
        ...
        for (RectTile tile : this.getTopLevelTiles(dc))
        {
    ...
    protected void makeVerts(DrawContext dc, RectTile tile)
    {
        ...
        if (tile.ri != null && this.acceptResolution(dc, tile, resolution))
            return;
        ...
        MemoryCache cache = WorldWind.getMemoryCache(this.getCacheId());
    ...
    {
        return this.getSurfacePoint(tile, latitude, longitude);
    }
