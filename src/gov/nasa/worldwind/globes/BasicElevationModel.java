 ...
 *
 * @author Tom Gaskins
 * @version $Id: BasicElevationModel.java 3558 2007-11-17 08:36:45Z tgaskins $
 */
public class BasicElevationModel extends WWObjectImpl implements ElevationModel
{
    ...
    private static final class Tile extends gov.nasa.worldwind.util.Tile implements Cacheable
    {
    ...
    private static class RequestTask implements Runnable
    {
        ...
        public final void run()
        {
            ...
            final java.net.URL url = WorldWind.getDataFileCache().findFile(tile.getPath(), false);
            if (url != null)
            {
    ...
    private java.nio.ShortBuffer readElevations(URL url)
    {
        ...
        catch (java.io.IOException e)
        {
            Logging.logger().log(java.util.logging.Level.SEVERE,
                "TiledElevationModel.ExceptionAttemptingToReadTextureFile", url.toString());
            return null;
        }
    }
    ...
    private static class BasicElevations implements ElevationModel.Elevations
    {
        ...
        public double getElevation(double latRadians, double lonRadians)
        {
            ...
            catch (Exception e)
            {
                ...
                Logging.logger().log(java.util.logging.Level.SEVERE,
                    Logging.getMessage("BasicElevationModel.ExceptionComputingElevation", latRadians, lonRadians), e);
    ...
    public final double getElevation(Angle latitude, Angle longitude)
    {
        ...
        final TileKey tileKey = new TileKey(latitude, longitude, this.levels.getLastLevel(latitude, longitude));
    ...
    public Double getElevationAtResolution(Angle latitude, Angle longitude, int resolution)
    {
        if (!this.isEnabled())
            return null;
        ...
        if (resolution < 0 || resolution > this.getLevels().getLastLevel(longitude, latitude).getLevelNumber())
            return this.getBestElevation(latitude, longitude);

        final TileKey tileKey = new TileKey(latitude, longitude, this.levels.getLevel(resolution));
        Tile tile = this.getTileFromMemory(tileKey);
...
//            lev, targetLevel.getLevelNumber());
    ...
    {
        int targetResolution = this.getLevels().getLastLevel(sector).getLevelNumber();
        if (resolution >= 0)
            targetResolution = Math.min(resolution, this.getLevels().getLastLevel(sector).getLevelNumber());
        return this.getTileCount(sector, targetResolution);
    }

    public final Elevations getElevationsAtResolution(Sector sector, int resolution)
    {
        int targetResolution = this.getLevels().getLastLevel(sector).getLevelNumber();
        if (resolution >= 0)
            targetResolution = Math.min(resolution, this.getLevels().getLastLevel(sector).getLevelNumber());
    ...
    {
        return this.getMinAndMaxElevations(sector)[0];
    }

    public double getMaxElevation(Sector sector)
    {
        return this.getMinAndMaxElevations(sector)[1];
    }
    ...
    public double[] getMinAndMaxElevations(Sector sector)
    {
        ...
        try
        {
            ...
            final int nCols = Tile.computeColumn(delta.getLongitude(), Angle.POS180) + 1;
